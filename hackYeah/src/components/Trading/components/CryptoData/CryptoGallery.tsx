...
const CryptoGallery: React.FC<CryptoGalleryProps> = (
  props: CryptoGalleryProps
) => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 ml-10 gap-4 p-4">
      {cryptos
        .filter((crypto) => crypto.name.indexOf(props.filter) != -1)
        .map((crypto: Crypto) => (
          <div
            key={crypto.id}
            ...
            onClick={() => props.onCryptoSelect(crypto.id)}
          >
            ...
            <div className="relative flex flex-col items-center justify-center h-full space-y-2">
              {" "}
              {/* Reduced space-y for less spacing */}
              <img
                src={crypto.image}
                alt={crypto.name}
