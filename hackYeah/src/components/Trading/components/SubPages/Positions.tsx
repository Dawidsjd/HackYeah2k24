  ...
  return (
    <div className="flex">
      ...
      <div className="positions mt-8 p-4 rounded-lg flex-grow">
        ...
        <Link to="/traiding" className="text-gray-400 hover:underline mb-4 flex">
  <ArrowLeft className="mr-2" /> {/* Dodanie klasy marginesu do prawej */}
  Back to Trading
</Link> {/* Dodaj przycisk */}
        {loading ? (
            ...
            <table className="table table-zebra w-full">
              ...
              <tbody>
                {positions.map((position, index) => {
                  ...
                  return (
                    <tr key={index} className="bg-gray-700 hover:bg-gray-600" onClick={() => openModal(position)}>
                      <td>
                      ...
                      <td className="text-white">${parseFloat(position.entryPrice.toString()).toFixed(2)}</td>
                      <td className="text-white">${parseFloat(position.currentPrice.toString()).toFixed(2)}</td>
        ...
        <div className="modal modal-open">
          <div className="modal-box p-6">
            <div className="flex justify-between items-center">
              <h2 className="text-lg font-bold">{selectedCoin.symbol.toUpperCase()}</h2>
              <button className="btn btn-sm btn-circle" onClick={closeModal}>✖</button>
            </div>
            <div className="flex justify-center my-6">
              <img src={selectedCoin.image} alt={`${selectedCoin.symbol} icon`} className="h-14 w-14" />
            </div>
            <div className="flex flex-col items-center my-4">
              {selectedCoin.currentPrice && selectedCoin.entryPrice ? (
                <div className={`text-xl font-bold ${selectedCoin.currentPrice > selectedCoin.entryPrice ? 'text-green-500' : 'text-red-500'}`}>
                  {selectedCoin.currentPrice > selectedCoin.entryPrice ? 'Zysk' : 'Strata '}:{' '}
                  {Math.abs((selectedCoin.currentPrice - selectedCoin.entryPrice).toFixed(2))} USD
                </div>
              ) : (
                <span className="text-gray-400">Brak danych o zysku/stracie</span>
              )}
            </div>
            <div className="flex justify-center">
              <button className="btn btn-warning w-64" onClick={closeModal}>Sell</button>
            </div>
          </div>
        </div>
      )}
